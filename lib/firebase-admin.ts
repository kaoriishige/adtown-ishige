...
// ----------------------------------------------------
// 初期化ステータスを保持する変数 (any型を使用し、型エラーを回避)
// ----------------------------------------------------
let initializedAdminAuth: any = null;
let initializedAdminDb: any = null;

/**
 * Firebase Admin SDKを初期化する。既に初期化されていれば何もしない。
 * 初期化に失敗した場合、Auth/DBオブジェクトはnullのままになる。
 */
 ...
 */
function _getAdminAuth(): admin.auth.Auth | any {
    if (!initializedAdminAuth) {
        initializeAdminApp(); 
        if (!initializedAdminAuth) {
 ...
 */
function _getAdminDb(): admin.firestore.Firestore | any {
...
export const adminAuth: admin.auth.Auth | any = {
    // 基本認証
...
} as admin.auth.Auth | any;

// adminDb.collection('settings') の形式で利用可能
export const adminDb: admin.firestore.Firestore | any = {
    // コレクション/ドキュメントアクセス
...
} as admin.firestore.Firestore | any;
