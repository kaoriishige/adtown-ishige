...
const RentalPage: NextPage<RentalProps> = ({ items }) => {
    ...
    return (
        <div className="min-h-screen bg-[#FDFCFD] pb-32 font-sans text-gray-900">
            ...
            <main className="max-w-xl mx-auto px-6 pt-6 animate-in fade-in duration-700">
                ...
                <section className="bg-gray-900 rounded-[2.5rem] p-8 text-white mb-8 shadow-xl relative overflow-hidden">
                    <div className="relative z-10">
                        ...
                        <p className="text-[10px] mt-4 font-bold text-pink-400 uppercase tracking-widest">Premium Members Only Service</p>
                    </div>
                ...
                <section className="grid grid-cols-2 gap-4">
                    {filteredItems.length > 0 ? (
                        filteredItems.map((item) => (
                            <Link key={item.id} href={`/premium/rental/${item.id}`} className="group bg-white rounded-[2.5rem] overflow-hidden shadow-sm border border-gray-100 flex flex-col active:scale-[0.97] transition-all hover:shadow-md">
                                ...
                                <div className="p-5 flex-1 flex flex-col justify-between">
                                    <h3 className="font-black text-sm text-gray-800 line-clamp-2 tracking-tight leading-snug mb-3">{item.title}</h3>
                                    <div className="space-y-2">
                                        ...
                                        <div className="flex items-center gap-1.5 text-gray-400">
                                            ...
                                            <p className="text-[9px] font-black truncate uppercase tracking-tighter">{item.location}</p>
                                        </div>
                                    </div>
                                </div>
                            </Link>
                        ...
                        <div className="col-span-2 py-24 text-center bg-gray-50/50 rounded-[3rem] border border-dashed border-gray-200">
                            ...
                            <p className="text-gray-400 text-[10px] font-black uppercase tracking-[0.2em]">現在、このカテゴリのアイテムはありません</p>
                            <Link href="/premium/rental/create" className="inline-block mt-4 text-[10px] font-black text-pink-500 border-b border-pink-500 pb-0.5">
                                最初のアイテムを登録する
                            </Link>
                        </div>
                    )}
                ...
                {/* INFO FOOTER */}
                <div className="mt-12 p-6 bg-pink-50/30 rounded-[2rem] border border-pink-100/50 flex items-start gap-4">
                    <RiInformationLine className="text-pink-400 shrink-0" size={20} />
                    <p className="text-[10px] font-bold text-pink-700/60 leading-relaxed italic">
                        那須レンタルはプレミアム会員限定のシェアリングサービスです。
                        対面での受け渡しが基本となります。ルールを守って楽しく活用しましょう。
                    </p>
                </div>
            </main>
        </div>
...
export const getServerSideProps: GetServerSideProps = async (context) => {
    try {
        ...
        const items = snapshot.docs.map(doc => ({ 
            id: doc.id, 
            ...doc.data(),
            // FirestoreのTimestamp型をシリアライズ可能な形式に変換
        ...
        console.error('Rental Page Error:', err);
        return { props: { items: [] } };
    }
};
