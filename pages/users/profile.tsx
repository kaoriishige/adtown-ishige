...
interface Profile {
    ...
    // AIマッチングエンジン出力用
    desiredAnnualSalary: number; // 💡 追加: 換算後の年収 (万円)
...
export const getServerSideProps: GetServerSideProps = async (context) => {
    try {
        ...
        // 🔴 修正点 1: データ読み込み先を 'users' から 'userProfiles' に変更
        ...
        const profile: Profile | null = profileData ? {
            ...
            desiredSalaryAmount: profileData.desiredSalaryAmount?.toString() || '',
            desiredWorkingHours: profileData.desiredWorkingHours || '',

            // AIマッチング用項目 (存在しない場合は 0 を設定)
            desiredAnnualSalary: profileData.desiredAnnualSalary || 0, 
...
const UserProfilePage: NextPage<ProfilePageProps> = ({ profile, uid }) => {
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        // 必須項目のチェック
        if (!formData.name || !formData.desiredSalaryAmount || !formData.desiredWorkingHours || formData.topPriorities.length === 0) {
             return setMessage({ type: 'error', text: '氏名、希望給与、勤務時間、TOP3のこだわりは必須項目です。' });
        }
        ...
        try {
            // 💡 修正点 3: 給与換算ロジックの追加
            let annualSalaryForAI = 0;
            const amount = Number(formData.desiredSalaryAmount);

            if (formData.desiredSalaryType === 'annual') {
                annualSalaryForAI = amount; // 年収 (万円)
            } else if (formData.desiredSalaryType === 'monthly') {
                annualSalaryForAI = amount * 12; // 月給 -> 年収 (万円)
            } else if (formData.desiredSalaryType === 'hourly') {
                // 時給 x 8時間 x 20日 x 12ヶ月 で年収に換算 (万円単位にするため 10000 で割る)
                annualSalaryForAI = Math.round((amount * 8 * 20 * 12) / 10000); 
            }
            
            // 保存するデータオブジェクトを作成
            const dataToSave = {
                ...formData,
                desiredAnnualSalary: annualSalaryForAI, // AIマッチング用データ
                updatedAt: serverTimestamp(),
            };
            
            // 🔴 修正点 2: データ保存先を 'users' から 'userProfiles' に変更
            await setDoc(doc(db, 'userProfiles', user.uid), dataToSave, { merge: true });
            
            setMessage({ type: 'success', text: 'プロフィールを保存しました。AIがあなたに最適な求人を探します。' });
            window.scrollTo(0, 0);
        } catch (err) {
            console.error("プロフィールの保存に失敗:", err);
            setMessage({ type: 'error', text: 'プロフィールの保存に失敗しました。Firebaseのセキュリティルールをご確認ください。' });
        } finally {
            setIsLoading(false);
        }
    };
                ...
                <form onSubmit={handleSave} className="bg-white p-8 rounded-lg shadow-md space-y-12">
                    ...
                    <section className="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea id="desiredWorkingHours" name="desiredWorkingHours" value={formData.desiredWorkingHours} onChange={handleChange} rows={3} required className="mt-1 block w-full input" placeholder="例：・平日の9時〜15時まで&#10;・週3日勤務希望&#10;・土日祝休み希望"></textarea>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                             ...
                             <div><label htmlFor="desiredLocation" className="block text-sm font-medium text-gray-700">希望勤務地</label><input type="text" id="desiredLocation" name="desiredLocation" value={formData.desiredLocation} onChange={handleChange} className="mt-1 block w-full input" placeholder="例：那須塩原市、リモート" /></div>
                        </div>
                    </section>
