...
import { adminAuth, adminDb } from '@/lib/firebase-admin'; // パスを絶対パスに修正
...
const HomePage: NextPage<HomePageProps> = ({ user }) => {
  ...
  // （...中略... あなたが作成したページのコンポーネント部分は完璧なので、そのまま記載します）
  // ジャンル
  const genres = [
...
// ここがビルドエラーの原因だった箇所。正しく修正されています。
export const getServerSideProps: GetServerSideProps = async (context) => {
  try {
    const cookies = nookies.get(context);
    // cookieの名前を'session'に仮定しています。必要に応じて変更してください。
    const token = await adminAuth.verifySessionCookie(cookies.session || '', true);
    const userDoc = await adminDb.collection('users').doc(token.uid).get();

    if (!userDoc.exists) {
      // Firestoreにユーザーデータがなければログインへ
    ...
    // cookieがない、無効など、何らかのエラーがあればログインページへ
    return { redirect: { destination: '/login', permanent: false } };
  }
};
