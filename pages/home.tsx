import { useState } from 'react'; // useCallback, useRouter を削除
...
// Firebaseクライアント側のインポート（TSエラー解消）
import { getAuth, signOut } from 'firebase/auth'; // Firebase公式SDKからインポート
import { app } from '@/lib/firebase-client'; // アプリ初期化をインポート
...
// ★★★ 修正済み LIFFによるURL遷移を処理する共通関数 ★★★
// isExternalSite: true の場合、LINEアプリ外のブラウザ（デフォルトブラウザ）で開く
// isExternalSite: false の場合、LIFFブラウザ内の新しいタブ/ビューで開く (Next.js内部リンク向け)
const handleLiffOpenWindow = (url: string, isExternalSite: boolean) => {
    const externalSetting = isExternalSite;

    if (typeof liff !== 'undefined' && liff.isInClient()) {
        try {
            liff.openWindow({
                url: url,
                external: externalSetting, 
            });
        } catch (error) {
            console.error('LIFF openWindow failed:', error);
            // エラー時はフォールバックとして通常のウィンドウを開く 
...
const HomePage: NextPage<HomePageProps> = ({ user }) => {
    ...
    const isStoreMatchingEnabled = new Date() >= FUTURE_ACCESS_DATE; 
    ...
    // ★★★ 最終修正: 友だち追加ボタンのハンドラー (LINE本体に戻る問題対策) ★★★
    const handleLineAddFriend = (e: React.MouseEvent) => {
        e.preventDefault();
        const lineUrl = 'https://lin.ee/Aac3C0d'; 
        
        // LINEディープリンクがLIFFセッションを強制終了させるのを防ぐため、
        // LIFFの openWindow ではなく、直接 window.open で外部ブラウザでの起動を試みる。
            ...
            <div className="bg-gray-100 min-h-screen">
                <div className="max-w-md mx-auto bg-white">
                    ...
                    <main className="p-4 space-y-6">
                        ...
                        {/* ▼▼▼ メインナビゲーションボタンセクション (アプリ内リンクはLinkを使用) ▼▼▼ */}
                        ...
                        <section className="bg-white pt-4 pb-2 px-4 rounded-xl shadow-sm">
                            ...
                            <div className="space-y-4">
                                {sponsors.map((sponsor) => (
                                    <button
                                        key={sponsor.name}
                                        // 外部サイトなので external: true を適用して開く
                                        onClick={() => handleLiffOpenWindow(sponsor.url, true)}
                                        className="w-full text-left p-0 bg-transparent border-none block group transition-opacity hover:opacity-80"
                                    >
                        ...
                        <footer className="text-center mt-8 pb-4 space-y-8">
                            ...
                            <section className="flex flex-col items-center gap-2">
                                ...
                                {/* 最終修正: handleLineAddFriend を使用 */}
                                <button 
                                    onClick={handleLineAddFriend}
                                    className="bg-transparent border-none p-0 inline-block"
                                >
                            ...
                            <div className="p-4 space-y-4 max-h-[60vh] overflow-y-auto">
                                {emergencyContacts.map((contact, index) => (
                                    <div key={`${contact.name}-${index}`} className="block p-3 bg-gray-50 rounded-lg">
                                        ...
                                        {/* 公式サイトへのリンクは external: true で開く */}
                                        <button 
                                            onClick={(e) => {
                                                e.preventDefault(); 
                                                handleLiffOpenWindow(contact.url, true);
                                            }}
                                            className="text-xs text-blue-500 hover:underline mt-1 inline-block bg-transparent border-none p-0"
                                        >
                                            公式サイトを見る
                                        </button>
                                    </div>
