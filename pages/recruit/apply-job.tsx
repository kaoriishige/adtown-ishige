...
interface UserProfile {
    ...
    desiredAnnualSalary: number; // AIマッチング用
}
interface Job {
    id: string;
    jobTitle: string;
    ...
    recruiterId: string; // 💡 必須: どの企業への応募か
}
interface ApplicantData {
    userId: string;
    recruitmentId: string;
    partnerId: string; // 企業のUID
...
export default function JobApplyPage() {
    ...
    const fetchRequiredData = useCallback(async (currentUser: User, jobId: string) => {
        try {
            ...
            // AIマッチングに必要な項目(desiredAnnualSalary)があるかチェック
    ...
    const handleApply = async () => {
        ...
        try {
            // 💡 修正: 応募ドキュメントをapplicantsトップレベルコレクションに作成
            ...
            // 2. 応募完了後、AIマッチングエンジンへの通知APIを呼ぶ (ここでは省略)
    ...
    if (!userProfile || !userProfile.desiredAnnualSalary) {
        return (
            <div className="p-10 text-center">
                ...
                <p className="mt-2 text-gray-600">応募には、AIマッチングに必要な**希望給与**を含むプロフィールを完成させてください。</p>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="border-t pt-4">
                                ...
                                <ul className="mt-3 text-sm space-y-1 p-3 bg-gray-50 rounded-md border">
                                    ...
                                    <li><span className="font-semibold">希望年収:</span> {userProfile?.desiredAnnualSalary}万円</li>
                                </ul>
                            </div>
