...
import * as admin from 'firebase-admin'; // Firestoreの型情報を使用
...
export const getServerSideProps: GetServerSideProps = async (context) => {
	try {
		// 汎用の認証関数 getUidFromCookie を使用
		const uid = await getUidFromCookie(context); 
		if (!uid) {
			// ログインしていない場合は広告パートナーログインページへリダイレクト
		...
		// ★★★ ログインループ解消のための権限チェックロジック ★★★
		// 求人パートナーとして許可するロール: 'recruit' (新), 'partner' (最旧)
		const allowedRecruitRoles = ['recruit', 'partner']; 

		const isRecruitAuthorized = userRoles.some(role => allowedRecruitRoles.includes(role));
		
		if (!isRecruitAuthorized) {
			// 権限がない場合、ログインページへリダイレクト（permission_deniedエラーを付与）
			return {
		...
		// TypeScriptエラー (7006) 修正: docに明示的に型を定義
...
const RecruitDashboardPage: NextPage<DashboardProps> = ({ companyName, initialSubscriptionStatus, jobs }) => {
	...
	const handleToggleSubscription = async () => {
		...
		try {
			...
			const newStatus = data.newStatus as 'active' | 'paused';
			setSubscriptionStatus(newStatus);
			
			// 全ての求人のローカルステータスも更新
			setJobList(prev => prev.map(job => ({ ...job, status: newStatus })));
			console.log(`求人広告サービス全体を${action === 'pause' ? '停止' : '再開'}しました。`);
		} catch (err: any) {
		...
		<div className="min-h-screen bg-gray-100">
			...
			<header className="bg-white shadow-sm">
				<div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-5 flex justify-between items-center">
					...
					<button onClick={handleLogout} className="flex items-center space-x-2 text-sm text-gray-600 hover:text-red-600 transition-colors bg-red-100 p-2 rounded-lg" aria-label="ログアウト">
