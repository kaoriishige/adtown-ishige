...
interface RecruitPartnerPageProps {
  user: UserProps | null;
}
...
const RecruitPartnerPage: NextPage<RecruitPartnerPageProps> = ({ user }) => {
    const [companyName, setCompanyName] = usePersistentState('recruitForm_companyName', user ? user.displayName : '');
    ...
    const [email, setEmail] = usePersistentState('recruitForm_email', user ? user.email : '');
    const [confirmEmail, setConfirmEmail] = usePersistentState('recruitForm_confirmEmail', user ? user.email : '');
    ...
    const handleAddService = async () => {
        ...
        try {
            ...
            if (!currentUser) { throw new Error('ログイン状態ではありません。再ログインしてください。'); }

            const idToken = await currentUser.getIdToken();

            const response = await fetch('/api/users/add-service', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${idToken}`},
                body: JSON.stringify({ service: 'recruit' }),
            });
            
            const data = await response.json();
            if (!response.ok) { throw new Error(data.error || 'サービスの追加に失敗しました。'); }
            ...
            <div className="bg-gray-50 text-gray-800 font-sans">
                ...
                <main className="container mx-auto px-6 py-20 text-center">
                    <div className="max-w-2xl mx-auto bg-white p-12 rounded-xl shadow-lg">
                        <ZapIcon className="w-16 h-16 mx-auto text-orange-500 mb-6" />
                        <h2 className="text-2xl font-bold">ようこそ、{user.displayName}様</h2>
                        <p className="mt-4 text-gray-600">
                            AIマッチング求人サービスを追加して、採用活動を始めませんか？<br/>
                            月額3,300円で、AIが貴社に最適な人材を自動で提案します。
                        </p>
                        <div className="bg-blue-100 border-l-4 border-blue-500 text-blue-800 p-4 my-8 rounded-md text-left">
                            <p className="font-bold">最初の課金は【2025年11月1日】です。</p>
                            <p className="text-sm">それ以前に料金が発生することはありません。いつでもキャンセル可能です。</p>
                        </div>
                        {error && ( <div className="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 my-6 rounded-md text-left"><p>{error}</p></div> )}
                        <button 
                            onClick={handleAddService} 
                            disabled={isLoading}
                            className="w-full py-4 mt-4 text-white text-lg font-bold bg-gradient-to-r from-orange-500 to-red-500 rounded-lg hover:shadow-lg disabled:opacity-50 transition-all duration-300"
                        >
                            {isLoading ? '処理中...' : 'AI求人サービスを追加する'}
                        </button>
                    </div>
                </main>
            </div>
                ...
                <section ref={registrationFormRef} id="registration-form" className="mt-24 pt-10">
                    <div className="bg-white p-8 md:p-12 rounded-2xl shadow-2xl w-full max-w-3xl mx-auto border border-gray-200">
                        ...
                        <p className="text-sm text-center mt-6">
                            すでにアカウントをお持ちですか？ <Link href="/login" className="text-orange-600 hover:underline font-medium">ログインはこちら</Link>
                        </p>
                    </div>
                </section>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="bg-white p-6 rounded-lg max-w-2xl w-full max-h-[90vh] flex flex-col">
                        ...
                        <div className="overflow-y-auto space-y-4 pr-2 text-sm">
                            ...
                            <p><strong>第10条（準拠法・裁判管轄）</strong><br/>本規約の解釈にあたっては、日本法を準拠法とします。本サービスに関して紛争が生じた場合には、当社の本店所在地を管轄する裁判所を専属的合合意管轄とします。</p>
