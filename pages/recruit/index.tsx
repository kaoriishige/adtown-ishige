// ファイル2: pages/recruit/index.tsx (修正済みの完全版)
...
const RecruitPartnerPage: NextPage = () => {
    ...
    const handleCopy = (text: string, field: string) => { navigator.clipboard.writeText(text).then(() => { setCopied(field); setTimeout(() => setCopied(''), 2000); }).catch(err => { console.error('コピーに失敗しました', err); }); };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setIsLoading(true);
        try {
            // ★★★ 呼び出すAPIの宛先を、新規登録専用のものに変更しました ★★★
            const response = await fetch('/api/recruit/register-and-subscribe', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ companyName, address, contactPerson, phoneNumber, email, password }),
            });
            
            const data = await response.json();

            // API側でエラーがあった場合は、それを表示します
            if (!response.ok) {
    ...
    return (
      // ... 以下、ページの見た目部分は変更がないため、すべてそのままです ...
        <div className="bg-gray-50 text-gray-800 font-sans">
            ...
            <main className="container mx-auto px-6">
                <section className="text-center py-16 md:py-24">
                    <p className="text-orange-500 font-semibold">【高い求人広告費を払い続けるのは、もうやめにしませんか？】</p>
                ...
                <section ref={registrationFormRef} id="registration-form" className="mt-24 pt-10">
                    <div className="bg-white p-8 md:p-12 rounded-2xl shadow-2xl w-full max-w-3xl mx-auto border border-gray-200">
                        <div className="text-center mb-10"><ZapIcon className="w-12 h-12 mx-auto text-orange-500 mb-4" /><h2 className="text-3xl font-bold text-center mb-2">求人パートナー登録</h2><p className="text-center text-gray-600">アカウントを作成し、月額3,300円のプランにお申し込みください。</p></div>
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="pt-4">
                                <label className="flex items-start">
                                    ...
                                    <span className="ml-3 text-sm text-gray-600"><button type="button" onClick={() => setShowTerms(true)} className="text-blue-600 hover:underline">利用規約</button>に同意し、AIマッチング求人サービスへ申し込みます。</span>
                                </label>
                            </div>
                       ...
                       <p className="text-sm text-center mt-6">すでにアカウントをお持ちですか？ <Link href="/recruit/login" className="text-orange-600 hover:underline font-medium">ログインはこちら</Link></p>
                    </div>
                </section>
                <section className="mt-20 bg-white rounded-2xl shadow-xl p-8 md:p-12 border border-gray-200 text-center">
                    ...
                    <p className="text-gray-600 mb-6">年間39,600円（税込）の一括前払いで、1年間求人を出し放題になるお得なプランです。<br />ご希望の方は、まず<strong className="text-orange-600">上記のフォームにご入力の上</strong>、以下のボタンからお問い合わせください。</p>
                            ...
                            <div className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div><label className="block text-sm font-medium text-gray-700 mb-1">本文:</label><textarea readOnly value={invoiceEmailContent.body} rows={8} className="w-full px-3 py-2 border rounded-md bg-gray-100" /><button onClick={() => handleCopy(invoiceEmailContent.body, 'body')} className="mt-1 w-full px-3 py-2 bg-gray-200 rounded-md text-sm hover:bg-gray-300">{copied === 'body' ? 'コピー完了' : 'コピー'}</button></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="bg-white p-6 rounded-lg max-w-2xl w-full max-h-[90vh] flex flex-col">
                        ...
                        <div className="overflow-y-auto space-y-4 pr-2">
                            ...
                            <p><strong>第3条（利用料金と支払方法）</strong><br/>1. 本サービスの利用料金は、月額3,300円（税込）とします。<br/>2. 支払方法はクレジットカード決済のみとし、登録希望者は当社が指定する決済代行サービスを通じて支払うものとします。<br/>3. 利用契約は毎月自動的に更新されるものとし、利用者はいつでも次回の更新をキャンセルすることができます。</p>
                        </div>
