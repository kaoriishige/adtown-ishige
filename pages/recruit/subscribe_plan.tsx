...
try {
  ...
  if (paymentMethod === 'invoice') {
    ...
    if (data.pdfUrl) {
      ...
      // alert() をカスタム UI に置き換えるべきですが、ここでは元のコードの形式を維持し、ユーザーに情報伝達を優先します。
        ...
        try {
            ...
            if (result.success) {
                if (isInvoice) {
                    ...
                    // Stripeリダイレクト成功時
                    // onCheckoutSuccess('card'); // この行は Stripe リダイレクトにより通常到達しない
                }
            }
        } catch (e) {
...
const RecruitSubscribePage: React.FC = () => {
    ...
    const [isAuthLoading, setIsAuthLoading] = useState(true); // ★ 認証状態のロードを明確化
    ...
    const { session_id, success } = router.query; // ★ Stripeのクエリパラメータを取得
    ...
    useEffect(() => {
        const unsub = onAuthStateChanged(auth, async (currentUser) => {
            setUser(currentUser);
            setIsAuthLoading(false); // 認証ロード完了
    ...
    // ★★★ 修正: 決済成功時のトークンリフレッシュとダッシュボードへのリダイレクト処理 ★★★
    useEffect(() => {
        // 1. Stripe Checkoutから戻ってきたかどうかをチェック
        if (session_id && success === 'true') {
            
            // 2. 認証が完了しているか、かつまだリダイレクト処理を開始していないかチェック
            if (!isAuthLoading && user) {
                ...
                const handlePostCheckout = async () => {
                    ...
                    // 1. Firebase IDトークンを強制リフレッシュ
                    try {
                        // トークンがリフレッシュされたら、サーバーは最新の isPaid: true の情報を持つ
                    ...
                    // 2. サーバーサイドのデータが更新されたことを期待し、ダッシュボードへリダイレクト
                    // リダイレクト先に payment_status=success をつけて、ダッシュボード側で成功メッセージを表示させる
                    router.replace('/recruit/dashboard?payment_status=success');
                };

                // この useEffect は query が変更されるたびに再評価されるため、一度処理を開始したら再度実行しないように
                // query をクリアするロジックがダッシュボード側にないため、ここでは実行を許可する。
                // ただし、連続実行を防ぐため、リダイレクト処理をすぐに開始する。
                handlePostCheckout();
            
            } else if (!isAuthLoading && !user) {
                 // 決済成功のクエリがあるがユーザーが認証されていない場合は、ログインページに飛ばす
                 router.replace('/partner/login');
            }
        }
    // 依存配列に isAuthLoading と user を追加し、認証が確定した瞬間にロジックが実行されるようにする
    }, [session_id, success, isAuthLoading, user, router]); 
