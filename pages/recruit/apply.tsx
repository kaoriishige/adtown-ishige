...
// ★★★ 修正: storeName を削除し、companyName に統一 ★★★
    ...
    try {
        ...
        if (userDocSnap.exists()) {
            ...
            if (roles.includes('recruit')) {
                ...
                router.push('/recruit/dashboard'); // 修正: /recruit/dashboard が正しいパスと仮定
                return; 
            }
        }
    } catch (e: any) {
...
}, [router, setAddress, setCompanyName, setContactPerson, setPhoneNumber, setEmail, setConfirmEmail]); // ★★★ 修正: setStoreName を削除 ★★★
...
* 既存の広告パートナーの場合、ロールの追加のみを実行するAPIを叩く
*/
...
companyName: companyName, // ★ 修正: companyName を使用
address,
area,
contactPerson,
phoneNumber,
