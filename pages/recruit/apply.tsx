...
const AddRecruitServicePage: NextPage = () => {
    ...
    useEffect(() => {
        ...
        // 認証状態の監視とデータ取得
        const unsubscribe = onAuthStateChanged(auth, async (user: User | null) => { // ★ 修正: user 型を明示
            if (user && user.email) {
                setUserId(user.uid); 
                try {
                    ...
                    for (const ref of refs) {
                        ...
                        if (userDocSnap.exists()) {
                            userData = userDocSnap.data(); // ★ 修正: 'data' ではなく 'userData'
                            console.log(`Fetched data from ${ref.path}:`, userData); // ★ 修正: 'userData' をログ
                            break;
                        }
                    }

                    if (userData) { // ★ 修正: 'data' ではなく 'userData'
    ...
    // ★ 修正: ESLintエラー(prefer-const)のため、useEffectを分離
    useEffect(() => {
        // 住所からエリアを自動判定
        if (address) {
            ...
            <main className="container mx-auto px-6">
                ...
                <section className="mt-24">
                    ...
                    <div className="mt-12 grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                        <div className="text-center p-6 bg-white rounded-lg shadow-lg"><div className="bg-orange-500 text-white w-16 h-16 rounded-full flex items-center justify-center text-2xl font-bold mx-auto mb-4">1</div><h4 className="text-xl font-bold">カンタン求人作成</h4><p className="mt-2 text-gray-600">...</p></div>
                        <div className="text-center p-6 bg-white rounded-lg shadow-lg"><div className="bg-orange-500 text-white w-16 h-16 rounded-full flex items-center justify-center text-2xl font-bold mx-auto mb-4">2</div><h4 className="text-xl font-bold">AIが候補者を自動提案</h4><p className="mt-2 text-gray-600">...</p></div>
                        <div className="text-center p-6 bg-white rounded-lg shadow-lg"><div className="bg-orange-500 text-white w-16 h-16 rounded-full flex items-center justify-center text-2xl font-bold mx-auto mb-4">3</div><h4 className="text-xl font-bold">いつでも更新を停止</h4><p className="mt-2 text-gray-600">...</p></div>
                    </div>
                </section>
                <section className="mt-20 bg-white rounded-2xl shadow-xl p-8 md:p-12 border border-gray-200">
