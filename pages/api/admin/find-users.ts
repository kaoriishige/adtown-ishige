...
export default async function handler(req: NextApiRequest, res: NextApiResponse<SuccessResponse | ErrorResponse>) {
  ...
  try {
    ...
    try {
      ...
      if (userDoc.exists) {
        foundUsers.push({ uid: userByEmail.uid, email: userByEmail.email!, name: userDoc.data()?.name, points: userDoc.data()?.points });
      }
    } catch (e) {
      try {
        const userById = await auth.getUser(query);
        const userDoc = await adminDb.collection('users').doc(userById.uid).get();
        if (userDoc.exists) {
          foundUsers.push({ uid: userById.uid, email: userById.email!, name: userDoc.data()?.name, points: userDoc.data()?.points });
        }
      } catch (e2) {
      }
    }
