  ...
  const handleLoginSuccess = async (user: User) => {
    ...
    try {
      ...
      if (response.ok && data.redirect) {
        // ★ スマホ固まり対策：router ではなく replace
        window.location.replace(data.redirect);
        return;
      }

      // 失敗時は強制サインアウト（無限ループ防止）
      const msg = data.error === 'user_data_missing'
        ? 'ユーザー登録が見つかりません。'
