...
const PartnerSignupPage: NextPage = () => {
  ...
  useEffect(() => {
    if (mainCategory) {
      ...
      setSelectedSubCategories([]); // 大分類が変わったら小分類の選択をリセット
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (selectedSubCategories.length === 0) {
      setError('カテゴリ（小分類）を1つ以上選択してください。');
      setIsLoading(false);
      return;
    }
      ...
      <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-lg">
        {isSuccess ? (
          ...
          <div className="text-center">
            <svg className="mx-auto h-16 w-16 text-green-500" /* ... */ >
          ...
          <>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              {mainCategory && (
                <div>
                  <label className="block text-gray-700 font-medium mb-2">カテゴリ（小分類）※複数選択可</label>
                  <div className="mt-2 p-4 border rounded-lg grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2">
                    {subCategoryOptions.map(subCat => (
                      <label key={subCat} className="flex items-center space-x-3 cursor-pointer">
                        <input type="checkbox" className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500" checked={selectedSubCategories.includes(subCat)} onChange={() => handleSubCategoryChange(subCat)} />
