...
import { getPartnerInfoFromFirebase } from '../../lib/firebase-admin';

// --- インラインSVGアイコンコンポーネント (元のコードから完全コピー) ---
...
const XCircleIcon = (props: React.SVGProps<SVGSVGElement>) => (
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}><circle cx="12" cy="12" r="10"></circle><line x1="15" y1="9" x2="9" y2="15"></line><line x1="9" y1="9" x2="15" y2="15"></line></svg>
);
...
export const getServerSideProps: GetServerSideProps<PageProps> = async (context) => {
    try {
        // ★★★★★【最重要】ここにFirebaseからUIDをコピー＆ペーストしてください ★★★★★
        const DEVELOPER_UID = "ここにFirebaseコンソールからコピーしたUIDを貼り付け";
        // ★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
        
        const partnerUid = DEVELOPER_UID;

        if (!partnerUid || partnerUid.includes("ここに")) {
            return { props: { error: "開発用UIDがコードに設定されていません。ファイルを修正してください。" } };
        }

        const partnerData = await getPartnerInfoFromFirebase(partnerUid);
        if (!partnerData) {
            return { props: { error: `指定されたUID (${partnerUid}) のパートナー情報がFirebaseに見つかりません。` } };
        }
        ...
        const partnerInfo: PartnerInfo = {
            ...
            mainCategory: partnerData.category?.main || '',
            selectedSubCategory: partnerData.category?.sub || '',
            qrStandCount: partnerData.qrStandCount || 0,
        };

        return { props: { partnerInfo } };
    } catch (error) {
        console.error("getServerSidePropsでエラー発生:", error);
        return { props: { error: "サーバーでデータ取得中にエラーが発生しました。" } };
    }
};
...
const PartnerSubscribePage: NextPage<PageProps> = ({ partnerInfo, error: serverError }) => {
    ...
    const handleInvoiceClick = () => {
        ...
        const body = `以下の情報でお申し込みいたします。\n担当者より折り返しご連絡ください。\n\n・店舗名／企業名：${partnerInfo.storeName}\n・ご担当者名：${partnerInfo.contactPerson}\n・お電話番号：${partnerInfo.phoneNumber}\n・メールアドレス：${partnerInfo.email}\n・住所：${partnerInfo.address}`;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        setError(null);
        if (!stripePromise || !partnerInfo) { setStripeError(true); return; }
        ...
        try {
            const response = await fetch('/api/partner/create-checkout-session', {
                ...
                body: JSON.stringify({ partnerId: partnerInfo.uid }),
            });
        ...
        return (
            <div className="min-h-screen flex flex-col justify-center items-center bg-gray-50 p-4">
                <div className="bg-white p-8 rounded-lg shadow-xl text-center max-w-2xl border">
                    <h2 className="text-2xl font-bold mb-4 text-red-600">設定エラー</h2>
                    <p className="text-gray-800 mb-6 bg-red-50 p-4 rounded-md text-left">
                        <strong>エラー内容:</strong> {serverError || "パートナー情報を取得できませんでした。"}
                    </p>
                    <p className="text-gray-600">このページを表示するには、コード内の`DEVELOPER_UID`をFirebaseコンソールの実際のUIDに設定する必要があります。</p>
                </div>
            </div>
        );
    }
    
    // --- ここから下がページの本体（HTML/JSX）。ご提示のコードと完全に同一の構造です ---
    return (
            ...
            <main className="container mx-auto px-6">
                ...
                <section className="mt-20 max-w-4xl mx-auto">
                    ...
                    <div className="mt-8 bg-white p-4 md:p-8 rounded-2xl shadow-xl border">
                        ...
                        <FAQItem question="紹介料は本当に毎月振り込まれますか？"><p className="leading-relaxed">はい、もちろんです。紹介料は月末締めで計算し、翌月15日にご登録いただいた銀行口座へ自動でお振込みいたします。振込額が3,000円に満たない場合は翌月に繰り越されますが、報酬が消えることはございませんのでご安心ください。</p></FAQItem>
                ...
                <section ref={registrationFormRef} id="registration-form" className="mt-20 pt-10">
                    <div className="bg-white p-8 md:p-12 rounded-2xl shadow-2xl w-full max-w-3xl mx-auto border border-gray-200">
                        <div className="text-center mb-10">
                            <p className="text-gray-700 leading-relaxed">ここまでお読みいただきありがとうございます。<br/>限定100店舗の初期費用無料キャンペーン枠は、すぐに埋まってしまうことが予想されます。<br/><strong className="text-red-600 font-bold">このチャンスを逃せば、101店舗目からは初期費用が発生いたします。もし1年間で得られた紹介手数料の合計が、年間のパートナー費用（39,600円）に満たなかった場合、お支払いいただいた費用を全額返金いたします。</strong><br/></p>
                        </div>
                        <h2 className="text-3xl font-bold text-center mb-2">パートナー登録 & 掲載お申し込み</h2>
                        <p className="text-center text-gray-600 mb-8">ご登録済みの情報が自動入力されています。内容をご確認の上、決済へお進みください。</p>
                        
                        <form onSubmit={handleSubmit} className="space-y-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div> <label className="block text-gray-700 font-medium mb-2">店舗名・企業名 *</label> <input type="text" value={partnerInfo.storeName} readOnly className="w-full px-4 py-2 border rounded-lg bg-gray-100 focus:outline-none"/> </div>
                                <div> <label className="block text-gray-700 font-medium mb-2">ご担当者名 *</label> <input type="text" value={partnerInfo.contactPerson} readOnly className="w-full px-4 py-2 border rounded-lg bg-gray-100 focus:outline-none"/> </div>
                            </div>
                            <div> <label className="block text-gray-700 font-medium mb-2">住所 *</label> <input type="text" value={partnerInfo.address} readOnly placeholder="例：栃木県那須塩原市共墾社108-2" className="w-full px-4 py-2 border rounded-lg bg-gray-100 focus:outline-none"/> </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div> <label className="block text-gray-700 font-medium mb-2">カテゴリ（大分類）*</label> <input type="text" value={partnerInfo.mainCategory} readOnly className="w-full px-4 py-2 border rounded-lg bg-gray-100 focus:outline-none" /> </div>
                                <div> <label className="block text-gray-700 font-medium mb-2">QRコードスタンド希望個数 *</label> <input type="number" value={partnerInfo.qrStandCount} readOnly className="w-full px-4 py-2 border rounded-lg bg-gray-100 focus:outline-none"/> </div>
                            </div>
                            <div> <label className="block text-gray-700 font-medium mb-2">カテゴリ（小分類）*</label> <input type="text" value={partnerInfo.selectedSubCategory} readOnly className="w-full px-4 py-2 border rounded-lg bg-gray-100 focus:outline-none" /></div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t">
                                <div> <label className="block text-gray-700 font-medium mb-2">電話番号 *</label> <input type="tel" value={partnerInfo.phoneNumber} readOnly placeholder="例: 09012345678" className="w-full px-4 py-2 border rounded-lg bg-gray-100 focus:outline-none"/> </div>
                                <div> <label className="block text-gray-700 font-medium mb-2">メールアドレス *</label> <input type="email" value={partnerInfo.email} readOnly className="w-full px-4 py-2 border rounded-lg bg-gray-100 focus:outline-none"/> </div>
                            </div>
                            <div className="pt-4">
                            ...
                            <button type="submit" disabled={isLoading || !agreed || stripeError} className="w-full py-4 mt-4 text-white text-lg font-bold bg-gradient-to-r from-orange-500 to-red-500 rounded-lg hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300">{getButtonText()}</button>
                ...
                <section className="mt-20 bg-white rounded-2xl shadow-xl p-8 md:p-12 border border-gray-200 text-center">
                    ...
                    <p className="text-gray-600 mb-6">御請求書にてのお支払いについては、年間39,600円を前払にてのご精算になります。<br />ご希望の方は、まず<strong className="text-orange-600">上記のフォームにご入力の上</strong>、以下のボタンからお問い合わせください。</p>
                    <button onClick={handleInvoiceClick} className="inline-block w-full max-w-md py-4 text-white text-lg font-bold bg-gradient-to-r from-teal-500 to-cyan-500 rounded-lg hover:shadow-lg transition-all duration-300">請求書払いのためのメールを作成</button>
                    {showInvoiceEmail && (
                        <div className="mt-6 p-4 border rounded-lg bg-gray-50 max-w-3xl mx-auto text-left">
                            ...
                            <div className="space-y-4">
                                <div className="text-center"><p className="text-sm mb-2">① 下のボタンをクリックして、メールソフトを起動してください。</p><a href={`mailto:1st.market.c@gmail.com?subject=${encodeURIComponent(invoiceEmailContent.subject)}&body=${encodeURIComponent(invoiceEmailContent.body)}`} className="bg-orange-500 text-white px-6 py-3 rounded-lg hover:bg-orange-600 inline-block font-bold">メールソフトを起動して送信する</a></div>
                                ...
                                <div>
                                    ...
                                    <div className="space-y-3">
                                        <div className="flex items-center"><label className="w-20 text-sm font-medium text-gray-700">宛先:</label><input type="text" readOnly value="1st.market.c@gmail.com" className="flex-1 px-3 py-2 border rounded-md bg-gray-100" /><button onClick={() => handleCopy('1st.market.c@gmail.com', 'to')} className="ml-2 px-3 py-2 bg-gray-200 rounded-md text-sm hover:bg-gray-300">{copied === 'to' ? 'コピー完了' : 'コピー'}</button></div>
                                        <div className="flex items-center"><label className="w-20 text-sm font-medium text-gray-700">件名:</label><input type="text" readOnly value={invoiceEmailContent.subject} className="flex-1 px-3 py-2 border rounded-md bg-gray-100" /><button onClick={() => handleCopy(invoiceEmailContent.subject, 'subject')} className="ml-2 px-3 py-2 bg-gray-200 rounded-md text-sm hover:bg-gray-300">{copied === 'subject' ? 'コピー完了' : 'コピー'}</button></div>
                                        <div><label className="block text-sm font-medium text-gray-700 mb-1">本文:</label><textarea readOnly value={invoiceEmailContent.body} rows={10} className="w-full px-3 py-2 border rounded-md bg-gray-100" /><button onClick={() => handleCopy(invoiceEmailContent.body, 'body')} className="mt-1 w-full px-3 py-2 bg-gray-200 rounded-md text-sm hover:bg-gray-300">{copied === 'body' ? 'コピー完了' : 'コピー'}</button></div>
                                    </div>
                                </div>
                            </div>
                        </div>
