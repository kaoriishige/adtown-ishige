...
// 🚨 修正: FirebaseクライアントSDKをインポート
import { getAuth, onAuthStateChanged, User } from 'firebase/auth';
import { doc, getDoc } from 'firebase/firestore';
import { auth, db } from '../../lib/firebase'; // 🚨 client-side SDKへのパス
...
const PartnerSignupPage = () => {
    ...
    useEffect(() => {
            ...
            if (user) {
                ...
                try {
                    ...
                    if (userDoc.exists()) {
                        ...
                        setStoreName(data.companyName || data.storeName || '');
        ...
        return () => unsubscribe();
    }, [setStoreName, setAddress, setContactPerson, setPhoneNumber, setEmail, setConfirmEmail, setPassword]);
    ...
    const isFormValid = !!(
        ...
        email && 
        confirmEmail && 
        (isUserLoaded || password.length >= 6) && // ログイン済みならパスワード不要
        area && 
        agreed && 
        email === confirmEmail
    );
                ...
                <section className="mt-24 max-w-4xl mx-auto">
                    ...
                    <div className="mt-8 bg-white p-4 md:p-8 rounded-2xl shadow-xl border">
                        ...
                        <FAQItem question="紹介手数料はどのように支払われますか？">
                            <p className="leading-relaxed">毎月末締めで計算し、翌々月15日にご指定の銀行口座へお振り込みします。振込額の合計が3,000円に満たない場合は、お支払いは翌月以降に繰り越されるものとします。パートナー様専用のダッシュボードで、いつでも収益状況をご確認いただけます。</p>
                        </FAQItem>
                ...
                <section ref={registrationFormRef} id="registration-form" className="mt-20 pt-10">
                    <div className="bg-white p-8 md:p-12 rounded-2xl shadow-2xl w-full max-w-3xl mx-auto border border-gray-200">
                        ...
                        <p className="text-center text-gray-600 mb-8">
                            ...
                            {/* 🚨 修正: ログイン済みの場合のメッセージ */}
                            {isUserLoaded ? (
                                <span className="block font-bold text-indigo-600 mt-2">求人パートナーとしてログイン中です。以下の情報を確認し、広告パートナーサービスを追加してください。</span>
                            ) : (
                                "すでに求人パートナーにお申込みの方は、こちらからは登録はできません。求人パートナーの管理画面からお申込みください。"
                            )}
                        </p>
                        <form className="space-y-6" onSubmit={(e) => e.preventDefault()}> {/* Prevent form submission */}
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t">
                                ...
                                <div> <label className="block text-gray-700 font-medium mb-2">メールアドレス *</label> <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required className={`w-full px-4 py-2 border rounded-lg focus:ring-orange-500 focus:border-orange-500 ${isUserLoaded ? 'bg-gray-100' : ''}`} disabled={isUserLoaded}/> </div>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="bg-white p-6 rounded-lg max-w-2xl w-full max-h-[90vh] flex flex-col">
                        ...
                        <div className="overflow-y-auto space-y-4 pr-2">
                            ...
                            <p><strong>第5条（紹介手数料）</strong><br />1. パートナーは、当社が提供する専用のQRコードを経由してアプリ利用者が有料会員登録を行った場合、当社所定の紹介手数料（以下「手数料」といいます。）を受け取ることができます。<br />2. 手数料は、有料会員の月額利用料金の30%とします。<br />3. 手数料は、月末締めで計算し、翌々月15日にご指定の銀行口座へお振り込みします。振込額の合計が3,000円に満たない場合は、お支払いは翌月以降に繰り越されるものとします。</p>
